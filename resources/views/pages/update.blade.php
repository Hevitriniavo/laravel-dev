...
@section('title', "Mise à jour d'une hôtel")

@section('content')
    <div class="container m-5 bg-transparent">
        <h1 class="text-center w-75 mb-5">Mise à jour Hôtel</h1>
        <form action="{{ route('hotel.update', $hotel->id) }}" method="POST" class="w-75 vstack" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <x-button type="submit" color="primary" label="Mettre à jour" />
        </form>
    </div>
@endsection
